...
import edu.berkeley.biocode.utils.IntuitiveStringComparator;
...
public class Rules {
    ...
    private List valids = new ArrayList();
    ...
    public Rules(bVWorksheet pWS1) {
        this.worksheet = pWS1;
    }

    public bVWorksheet getWorksheet() {
        return worksheet;
    }
    ...
    public void run(mainPage mp) {
        ...
        for (Iterator i = rule.iterator(); i.hasNext();) {
            ...
            Method method = null;
            try {
                //mp.insertMsg("Processing rule " + r.getType());
                //System.out.println("Processing rule " + r.getType());
                mp.insertMsg("Processing rule " + r.getType() + " for " + r.getName() + " (" + count + " of " + rule.size() + ")");
            ...
            if (method != null) {
                try {
                    ...
                    addMessage(r, "Application could not process " + r.getType() + " rule (" + r.getName() + ")");
                    e.printStackTrace();
    ...
    /**
     * Check values in 1 row against values in another row
     * either in the current sheet or in another sheet
     * NOTE: only can parse two sheets at the same time currently
     *
     * @param r
     */
    public void checkMatchingFieldNameInSecondarySheet(Rule r) {
        /* if (WS2 == null) {
            addMessage(r, "Collecting Event sheet not loaded, unable to run check on matching collecting event ID's here. (Will need to verify on upload to DB)");
            return;
        }

        if (worksheet.numRows > 2000 || WS2.numRows > 2000) {
            //addMessage(r, "Too many rows in sheets to process matchingfieldnameinSecondarySheet Rule -- only 2000 rows or less allowed here");
            mp.insertMsg("Too many rows in sheets to process matchingfieldnameinSecondarySheet Rule -- only 2000 rows or less allowed here");

        }
        try {
            String reqFieldName = "", drowValue = "", irowValue = "", field = "", msg = "";
            boolean booFound = false;

            // Check that photoListRow Data matches Field Definitions
            for (int j = 1; j <= worksheet.numRows; j++) {
                drowValue = worksheet.getStringValue(r.getName(), j);
                if (drowValue != null) {

                    booFound = false;
                    for (int k = 0; k <= WS2.numRows; k++) {
                        irowValue = WS2.getStringValue(r.getName(), k);

                        if (drowValue.equals(irowValue)) {
                            booFound = true;
                        }

                    }
                    if (!booFound) {
                        msg = "Did not find " + r.getName() + "=" + drowValue + " in " + r.getName() + " (" + WS2.getSheetName() + ").  NOTE: Did not check values in online database so you may ignore this warning if you are sure the collecting event values have already been loaded previously.";
                        addMessage(j, r, msg);
                    }
                }
            }
        } catch (Exception e) {
            addMessage(r, "checkMatchingFieldNameInSecondarySheet: " + e.toString());
            return;
        }
        */
    ...
    public void addMessage(int row, Rule r, String message) {
        //System.out.println(row + "," + mp.FieldHeadingsOnRow.getSelectedIndex());
